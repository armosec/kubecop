...
original_checksum=$(md5sum chart/kubecop/crds/runtime-rule-binding.crd.yaml | awk '{print $1}')

# Run md_gen
go run cmd/rule_md_gen/generate_md.go

# Store the new checksum of the file
new_checksum=$(md5sum chart/kubecop/crds/runtime-rule-binding.crd.yaml | awk '{print $1}')
